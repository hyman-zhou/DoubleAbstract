...
public class DeBlockAlgorithmOfMII_Limit {
    ...
    public void deBlockAlgorithmOfMii_Limit(){
        deBlock();
        try {
            checkSamePart();
        } catch (IOException e) {
    ...
    private JSONObject deBlockFromInputBuffer(BufferedInputStream in) throws IOException {
        ...
        while ((byteRead = in.read()) != -1) {
            ...
            if(byteRead>frontByte){
                ...
                if(incrementTimes>=lenOfWindow+1||distance>700&&incrementTimes>=lenOfWindow||distance>5000){
                    //找到新切点
...
//        try {
//            FileOutputStream fileOut = new FileOutputStream("e://a_LMI_LIMIT.txt");
...
//        }
        ...
        //System.out.printf("Time to deBlock with MII_Limit: %d ms. %d,%d,%d,%d,%d\n",endTime-startTime,testTimes1000,testTimes3000,testTimes5000,testTimes10000,testTimesBig);
        System.out.println("End of deblock by MII_LIMIT!");
        return jsonResult;
    }
    ...
    private void checkSamePart() throws IOException {
        ...
        for(String testFile:fileList){
            try {
                    ...
                    if(byteRead>frontByte){
                        ...
                        if(incrementTimes>=lenOfWindow+1||distance>700&&incrementTimes>=lenOfWindow||distance>5000){
                            ...
                            if(dataStroe_m.contains(blockString)){
                                distance=fileIndex-startIndex+1;
                                sameBytes+=distance;
                ...
                System.out.printf("MII_LIMIT---%s:发现重复数据 %d 个字节\n",testFile,sameBytes);
